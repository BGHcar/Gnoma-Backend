...
def process_file_parallel(file_path: str):
    ...
    with open(file_path, 'r') as f:
        while True:
            ...
            if not lines:  # No more lines to read
                break
                
            total_lines += len(lines)
            chunk_positions.append((pos, len(lines)))
            last_position = f.tell()  # Guardar la última posición

        # Verificar si hay líneas restantes después del último chunk completo
        f.seek(last_position)
        remaining_lines = f.readlines()
        if remaining_lines:
            total_lines += len(remaining_lines)
            chunk_positions.append((last_position, len(remaining_lines)))
