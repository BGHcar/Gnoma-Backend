...
@app.get("/genome/all")  # Define una ruta GET para obtener todas las variantes
async def get_all_variants(page: int = 1, page_size: int = 10):  # Define la función para manejar la solicitud
    filter = "CHROM"  # Establece el filtro por defecto
    search = ""  # Establece la búsqueda por defecto
    start_time = time.time()  # Obtiene el tiempo de inicio
    variants = await get_variants_parallel(
        query={filter: {"$regex": search, "$options": "i"}},  # Define la consulta
        sort_by=None,  # No ordena los resultados
        sort_order=1,  # Orden ascendente
        page=page,  # Página actual
        page_size=page_size,  # Tamaño de página
        hint=[("CHROM", 1)]  # Pista de índice
    )
    return {
        "data": variants,  # Devuelve los resultados
        "process_time": round(time.time() - start_time, 3)  # Devuelve el tiempo de procesamiento
    }

@app.get("/genome/search")  # Define una ruta GET para buscar variantes
async def search_variants(
    filter: str = "CHROM",  # Filtro por defecto
    search: str = "",  # Búsqueda por defecto
    page: int = 1,  # Página actual
    page_size: int = 10,  # Tamaño de página
    sort_by: str = None,  # Campo de ordenación por defecto
    order: str = None  # Orden por defecto
):
    start_time = time.time()  # Obtiene el tiempo de inicio
    query = {filter: {"$regex": search, "$options": "i"}}  # Define la consulta
    
    sort_order = 1 if order and order.upper() == "ASC" else -1  # Establece el orden de ordenación
    hint = None  # Inicializa la pista de índice
    
    if sort_by:  # Si se proporciona un campo de ordenación
        if sort_by.startswith("output_"):  # Si el campo de ordenación empieza con "output_"
            hint = [("output", 1)]  # Usa la pista de índice "output"
        else:
            hint = [(sort_by, 1)]  # Usa la pista de índice del campo de ordenación
    else:
        if filter in valid_fields:  # Si el filtro es un campo válido
            hint = [(filter, 1)]  # Usa la pista de índice del filtro
        else:
            hint = [("CHROM", 1)]  # Usa la pista de índice "CHROM"
    
    logging.info(f"Query: {query}, Sort by: {sort_by}, Sort order: {sort_order}, Hint: {hint}")  # Registra la consulta, orden y pista
    
    results = await get_variants_parallel(
        query=query,  # Consulta
        sort_by=sort_by,  # Campo de ordenación
        sort_order=sort_order,  # Orden de ordenación
        page=page,  # Página actual
        page_size=page_size,  # Tamaño de página
        hint=hint  # Pista de índice
    )
    
    return {
        "data": results,  # Devuelve los resultados
        "process_time": round(time.time() - start_time, 3)  # Devuelve el tiempo de procesamiento
    }

@app.get("/genome/sort")  # Define una ruta GET para ordenar variantes
async def sort_variants(
    sort_by: str = "CHROM",  # Campo de ordenación por defecto
    order: str = "ASC",  # Orden por defecto
    page: int = 1,  # Página actual
    page_size: int = 10,  # Tamaño de página
    filter: str = None,  # Filtro por defecto
    search: str = None  # Búsqueda por defecto
):
    logging.info(f"Filtros de búsqueda: {filter} - {search}")  # Registra los filtros de búsqueda
    start_time = time.time()  # Obtiene el tiempo de inicio
    
    if order not in ["ASC", "DESC"]:  # Si el orden no es válido
        return {
            "error": "Invalid order value. Use 'ASC' or 'DESC'.",
            "process_time": round(time.time() - start_time, 3)
        }
    
    if sort_by not in valid_fields:  # Si el campo de ordenación no es válido
        return {
            "error": f"Invalid sort_by value. Valid fields are: {', '.join(valid_fields)}",
            "process_time": round(time.time() - start_time, 3)
        }
    sort_order = 1 if order == "ASC" else -1  # Establece el orden de ordenación

    query = None  # Inicializa la consulta
    if filter and search:  # Si se proporcionan filtro y búsqueda
        query = {filter: {"$regex": search, "$options": "i"}}  # Define la consulta
        logging.info(f"Sort combined with search query: {query}")  # Registra la consulta combinada
        hint = [(filter, sort_order)]  # Usa la pista de índice del filtro
    else:
        if sort_by.startswith("output_"):  # Si el campo de ordenación empieza con "output_"
            hint = [("output", sort_order)]  # Usa la pista de índice "output"
        else:
            hint = [(sort_by, sort_order)]  # Usa la pista de índice del campo de ordenación
